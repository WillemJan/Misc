...
class harvest_identifiers():
    ...
    def loop_once(self):
        ...
        try:
            ...
            print("Got invalid XML, have a nice day!")
            os._exit(-1)
        ...
        for item in data.iter():
            ...
            if item.tag.find("identifier") > -1:
                if add:
                    ...
                    doc = { "id" : item.text, 
                            "status" : "new",
                            "_id" : hashlib.md5(item.text.encode('ascii','ignore')).hexdigest() } 
                    record = self.mongoDB[self.setName].find_one({"_id" : hashlib.md5(item.text.encode('ascii', 'ignore')).hexdigest()})
                    if record:
                    ...
                    if delete:
                        if delete["status"] == "new" or delete["status"] == "done":
                            self.mongoDB[self.setName].remove({"_id" : hashlib.md5(item.text).hexdigest()})
                    add = True
                    deleted += 1
...
class mongoDB_client():
    dbHandler = False
    def __init__(self, serverName="localhost", port=27017):
        try:
...
if __name__ == "__main__":
    mongoDB = mongoDB_client(serverName="192.87.165.3")

    sets = [ "DARE"]

    for name in sets:
        harvest = harvest_identifiers(mongoDB.dbHandler["OAI"], setName=name)

        while harvest.resumptionToken:
            harvest.loop_once()

        print("Total nr of records to be fetched : "+str(mongoDB.dbHandler["OAI"][name].find({"status" : "new"}).count()))
