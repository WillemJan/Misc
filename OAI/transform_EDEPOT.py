...
def post_url(data, colname="EDEPOT"):
    http = Http(timeout=10)
    headers, response = http.request("http://127.0.0.1:8080/solr/"+colname.lower()+"/update", method="POST",body=bytes(data),  headers={'Content-type': 'text/xml'}) 
    return(headers)
...
if __name__ == "__main__":
    ...
    count=mongoDB.dbHandler["OAI"][setname].find({"status" : "indexed"})
    total=count.count()

    template=libxml2.parseFile("edepot.xsl")
    ...
    if data.find('cannotDisseminateFormat') > -1:
