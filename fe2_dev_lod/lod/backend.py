...
socket.setdefaulttimeout(19300)
...
class Backend(dict):
    """ Backend for different Linked Open Data(LOD) sources.\n
    The backend is able to cache the fetched (raw)data into different backends.\n
    """
    ...
    def _get_nocache(self, url, mode=""):
        ...
        headers = {'Accept' : '*/*'}
        req = urllib2.Request(url=url.encode('utf8'), headers = headers)
        try:
            response = urllib2.urlopen(req )
        except urllib2.HTTPError, e:
            self.http_retries-=1
            if self.debug:
                self.log.info("Got an http error, sleeping for 10seconds, %s retries to go. (%s)" % (self.http_retries, e))
    ...
    def _get_pymongo(self, url, mode):
        ...
        myhash = hashlib.md5(url.encode('utf-8')).hexdigest()
        self.log.debug("Getting %s (via %s)" % (url, self.backend))
    ...
    def _add(self, identifier, value, namespace):
        ...
        if not namespace in self[identifier]:
        ...
        if (type(value) == unicode) or (type(value) == str):
            value= value.replace('http://dbpedia.org/resource/', 'http://dev.fe2.nl/DBP:').replace('http://dbpedia.org/ontology/', 'http://dev.fe2.nl/DBP:')
            if value.isdigit():
