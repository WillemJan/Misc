...
Usage: isbn <ISBN> 
    ...
    URL_OPENLIBRARY = "http://openlibrary.org/api/books?bibkeys=ISBN:%s&callback=mycallback"
    ...
    def _sameas(self):
        ...
        for item in xml.iterchildren():
            ...
            if not self.isbn == isbn and not self.isbn == isbn_check.convert(isbn):
                self._add("librarything_sameas", "http://dev.fe2.nl/isbn:"+isbn, "isbn")
    ...
    def _openlibrary_lookup(self):
        URL_OPENLIBRARY = "http://openlibrary.org/api/books?bibkeys=ISBN:%s&callback=mycallback" % (self.isbn)
        try:
