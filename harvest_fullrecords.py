...
if __name__ == "__main__":
    ...
    #harvest = harvest_identifiers(mongoDB.dbHandler["OAI"])


    for i in range(0,10):
        record=mongoDB.dbHandler["OAI"]["GVN"].find_one({"status" : "new"})
        record["status"] = "fetching"
        mongoDB.dbHandler["OAI"]["GVN"].save(record)

        try:
            data=urllib.urlopen("http://services.kb.nl/mdo/oai?verb=GetRecord&identifier="+record["id"]+"&metadataPrefix=dcx").read()
            record["status"] = "done"
            record["data"] = data
            mongoDB.dbHandler["OAI"]["GVN"].save(record)
            print("ok")
